	...
	for t in soup.body.descendants:
		...
		if visible_html_tag(t) and hasattr(t, 'string'):
			...
			for s in sentences:
				...
				replaced, alter_amount = replace_document_words(tx, tagged_text)
				replaced_string += " " + prettify_sentence(replaced)
				if alter_amount > 1: freudified.append(replaced_string)
...
def slip_sentences(source, url):
	'''
		Freudifies given source's individual sententes and returns them as in
		json format. 
	...
	s = timer()
	soup = bs(source, 'lxml')
	logger.info("Souped %s in %s" % (url, str(timer() - s)))
	
	try:
		t = timer()
		logger.info("Fixed relative paths of %s in %s" % (url, str(timer() - t)))
		t = timer()
		sentences = freudify_soup(soup)
		logger.info("Freudified %s in %s" % (url, str(timer() - t)))
	except:
		logger.info("utils.slip_sentences has blown, with stack trace: \n %s" % traceback.format_exc())
		return None
		
	logger.info("Finished tagging source from: %s " % url)
	return json.dumps({'slipped': sentences}, sort_keys=True, indent=4, separators=(',', ': '))

#	All part of speech tags as keys and words for the tags as values.
POS = read_wordnet(os.path.join(__file__, "sexuality.txt"))
